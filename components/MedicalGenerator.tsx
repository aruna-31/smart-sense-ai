...
export const MedicalGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Payment required bubble-up
      if (err?.status === 402 && err?.detail) {
        setSubscribeInfo(err.detail);
        setDescription(err.detail?.message || 'Subscription required to continue.');
      } else {
    ...
    <AnimatedPage>
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-400 mb-6 text-lg">
          An educational tool to generate simplified medical information for patients, or SAMPLE student medical note templates (educational use only).
        </p>
        
        <div className="bg-red-900/50 border border-red-700/80 text-red-200 p-4 rounded-lg mb-8">
          <p className="font-bold">Disclaimer:</p>
          <p className="text-sm">This is an AI-powered educational tool. Patient-facing tips are general information only and not medical advice; always consult a qualified healthcare professional for personal medical concerns. Student notes are SAMPLE templates for educational use only and are not real medical records.</p>
        </div>
          ...
          >
            ...
            {isLoading ? (
              ...
              <>
                ...
                {subscribeInfo && (
                  <div className="mt-4 bg-yellow-900/80 border border-yellow-700 p-3 rounded-lg text-yellow-100">
                    <div className="flex justify-between items-center">
                      <div>
                        <div className="font-semibold">Subscription required</div>
                        <div className="text-sm">{subscribeInfo.message ?? `Subscribe for ${subscribeInfo.amount} ${subscribeInfo.currency} / ${subscribeInfo.interval}`}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={() => setBillingOpen(true)} className="bg-green-600 px-3 py-1 rounded-md text-white">Subscribe</button>
                        <button onClick={() => setSubscribeInfo(null)} className="px-3 py-1 rounded-md text-gray-300">Dismiss</button>
                      </div>
                    </div>
                  </div>
                )}
