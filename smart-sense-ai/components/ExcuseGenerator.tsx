...
export const SmartExcuseGenerator: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        // Save to user activity history (non-blocking)
        try {
            const userId = localStorage.getItem('user-id') || 'anonymous';
            const inputText = activeTool === 'excuse' ? excuseSituation : activeTool === 'apology' ? apologySituation : '';
            const outputText = generatedResult.text || (generatedResult as any);
            await fetch('/api/activity', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, component: 'ExcuseGenerator', input: inputText, output: outputText }) });
        } catch (e) {
        ...
        try {
            const userId = localStorage.getItem('user-id') || 'anonymous';
            const input = activeTool === 'excuse' ? excuseSituation : (activeTool === 'apology' ? apologySituation : (activeTool === 'email' ? `${emailTo}|${emailSubject}|${emailPoints}` : `${letterTo}|${letterFrom}|${letterPoints}`));
            await saveActivity(userId, activeTool === 'excuse' ? 'ExcuseGenerator' : (activeTool === 'apology' ? 'ApologyGenerator' : (activeTool === 'email' ? 'EmailAssistant' : 'LetterWriter')), input, generatedResult.text ?? String(generatedResult));
        } catch (e) {
            console.warn('Failed to save activity', e);
        }
    };
